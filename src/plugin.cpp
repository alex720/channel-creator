...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"channel creator";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "channel creator";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
bool IstChannelNameFrei(uint64 serverConnectionHandlerID, char* wantedChannelName) {
	uint64 *result;
	char* currentChannelName;
	if (ts3Functions.getChannelList(serverConnectionHandlerID, &result) == ERROR_ok) {
		bool isEnd = false;
		int i = 0;
		while (result[i] != NULL) {
			if (ts3Functions.getChannelVariableAsString(serverConnectionHandlerID, result[i], CHANNEL_NAME, &currentChannelName) == ERROR_ok) {
			//	char tempStr[100] = "channelcreate channel_name=";
				//strcat(tempStr, currentChannelName);

				if (strcmp(currentChannelName, wantedChannelName) == 0) {
					return false;
				}
			}
			i++;
...
int std_channel_max_clients = 15;
int std_codec_quality = 6;
char* std_channelpw;
int std_talkpower;
char* std_channelname;
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
	...
	channelcreator(serverConnectionHandlerID,channelname,channelpw, talkpower);
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL, MENU_ID_GLOBAL_1, "Channel Erstellen", "1.png");
...
void ts3plugin_onUpdateChannelEditedEvent(uint64 serverConnectionHandlerID, uint64 channelID, anyID invokerID, const char* invokerName, const char* invokerUniqueIdentifier) {
	...
	ts3Functions.getClientVariableAsUInt64(serverConnectionHandlerID, myID, CLIENT_CHANNEL_GROUP_ID, &CGID);
	if (mychannelID != channelID || CGID != 10)
		return;
	...
	if (editor2 == invokerID) 
		ts3Functions.setchannelgroup(serverConnectionHandlerID, invokerID, mychannelID, 9);
	
	if (editor1 == invokerID)
		editor2 = invokerID;
		...
		case PLUGIN_MENU_TYPE_GLOBAL:
			...
			switch(menuItemID) {
				case MENU_ID_GLOBAL_1:
					...
					channelcreator(serverConnectionHandlerID, "shittys Talk", "");
